<template>
  <div>
    <div class="pay-order-header">
      ...
      <router-link class="paybutton-left fl" style="width: 40%;text-align: center;" to="/">
          取消
      </router-link>
...
<script>
  export default {
    ...
    created()
    {
      ...
      // 先判断是否为空，不然.cssh会报错。
      if(this.$store.state.user.ordercoupon!=null){
         this.cash=this.$store.state.user.ordercoupon.cash;
         this.couponid=this.$store.state.user.ordercoupon.id;
    ...
    methods:{
      ...
      selectAddr()
      {
        this.$router.push({path:'/addrlist',query:{action:"select"}})
      },
      // 优惠券
      selectcoupon()
      {
        this.$router.push({path:'/coupon',query:{action:"myseltct"}})
      },
      orderadd()
      {
        let data = {
          userid:this.$cookies.get('user').id,
          addrid:this.$store.state.user.orderaddr.id,
          couponid:this.couponid,
        }
        this.$store.dispatch('orderadd',data).then(res => {
          if(res.data.result)
          {
