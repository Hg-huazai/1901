...
export default {
	...
	created(){
	 //触发actions里面叫做menushow这个方法
	  this.$store.dispatch('menushow',true)
	   ...
	   // 一进来给他数据
      this.$store.dispatch('commenteditdata',{commentid:this.$route.query.commentid}).then(res => {
		this.userdata = res.data.data
		// this.userdata.level = res.data.data.level
  ...
  methods:{
      addcomment()
      {
        ...
        this.$store.dispatch('mycommentedit',this.userdata).then(res => {
          if(res.data.result)
          {
